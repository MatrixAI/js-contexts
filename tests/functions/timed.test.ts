...
describe('functions/timed', () => {
  ...
  describe('timed expiry', () => {
    ...
    test('async generator expiry', async () => {
      ...
      await testsUtils.sleep(50);
      await expect(g.next()).rejects.toThrow(errors.ErrorContextsTimedTimeOut);
    });
    test('generator expiry', async () => {
      ...
      expect(g.next()).toEqual({ value: 'hello world', done: false });
      await testsUtils.sleep(50);
      expect(() => g.next()).toThrow(errors.ErrorContextsTimedTimeOut);
    });
  });
  describe('timed propagation', () => {
    ...
    test('propagated expiry', async () => {
      ...
      await expect(fTimed()).rejects.toThrow(errors.ErrorContextsTimedTimeOut);
    });
  });
