...
import timedCancellable from '@/decorators/timedCancellable';
import * as errors from '@/errors';
import { AsyncFunction } from '@/utils';
import * as testsUtils from '../utils';

describe('decorators/timedCancellable', () => {
  ...
  describe('timedCancellable decorator expiry', () => {
    test('async function expiry - eager', async () => {
      const { p: finishedP, resolveP: resolveFinishedP } = testsUtils.promise();
      class C {
        /**
         * Async function
         */
      ...
      const c = new C();
      await expect(c.f()).rejects.toThrow(errors.ErrorContextsTimedTimeOut);
  ...
  describe('timedCancellable decorator explicit timer cancellation or signal abortion', () => {
    ...
    test('explicit signal signal abortion with passed in timer - during', async () => {
      ...
      const timer = new Timer({
        handler: () => {
          abortController.abort(new errors.ErrorContextsTimedTimeOut());
        },
        delay: 100,
      });
  ...
  describe('timedCancellable decorator dynamically setting parameters', () => {
    test('dynamically setting lazy parameter', async () => {
      let waitPromise = testsUtils.promise();
      let finished = false;
      ...
      finished = false;
      lazy = true;
      waitPromise = testsUtils.promise();
